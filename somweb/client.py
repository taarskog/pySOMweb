...
def _get_value_using_regex(content: str, regex: Pattern, group_name: str) -> str:
...
class SomwebClient:
    ...
    async def __aenter__(self):
        """
        Asynchronously enters the context manager and returns the current object.

        This method is part of the context manager protocol in Python. It is called when the `async with` statement is used to enter the context. It returns the current object, allowing it to be used within the `async with` block.
    ...
    async def __aexit__(self, *excinfo):
        ...
        This method is part of the context manager protocol in Python. It is called when the `async with` statement is used to exit the context. It returns None.
    ...
    async def is_alive(self):
        """
        Check if SOMweb device available and responding to requests (deprecated).
    ...
    async def async_is_alive(self) -> bool:
        """
        Check if SOMweb device available and responding to requests.
        ...
        try:
            ...
            return response.status < 400 and await response.text() == "1"
    ...
    async def authenticate(self):
        """
        Authenticate or re-authenticate (deprecated).
    ...
    async def wait_for_door_state(
        ...
        """
        Wait for a door to reach the requested state (deprecated).
    ...
    async def door_action(
        ...
        """
        Perform an action on a door (deprecated).
    ...
    async def toogle_door_position(self, door_id: int, token: str = None) -> bool:
        """
        Toggles a door postion (deprecated).
    ...
    async def async_toogle_door_position(self, door_id: int, token: str = None) -> bool:
        ...
        response = await self.__http_client.async_get(url)
        return response.status < 400 and await response.text() == "OK"
    ...
    async def async_get_device_info(self) -> DeviceInfo:
        ...
        try:
            ...
            return DeviceInfo(
                ...
                _get_value_using_regex(page_content, RE_WIFI_SIGNAL, "quality"),
                _get_value_using_regex(page_content, RE_WIFI_SIGNAL, "level"),
                _get_value_using_regex(page_content, RE_WIFI_SIGNAL, "unit"),
                _get_value_using_regex(page_content, RE_TIME_ZONE, "time_zone"),
            )
        except Exception as ex:  # noqa: BLE001
